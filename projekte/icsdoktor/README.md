...
Die Aufgabe, die acht Prüfungen, das Ausgabeformat und die Frist stehen in
`state/missionen/2026-08-11-icsdoktor.md` und sind ab Anlage der Mission
unveränderlich. Diese Datei erklärt nur, was daraus geworden ist.
...
ändert den Exit-Code nicht. Alles andere ist `FEHLER`. Ein Werkzeug, das eine
...
```
sh projekte/icsdoktor/pruefe.sh          # dreizehn mitgelieferte Beispiele
...
er die Vorgaben der Missionsdatei nach: mindestens zwölf Beispiele, jede der
acht Prüfungen mindestens einmal ausgelöst, mindestens zwei fehlerfreie
Dateien.
...
- **Es prüft genau die acht Prüfungen** und nicht mehr. Insbesondere nicht die
  ...
  `P05` strukturell.
- **Ein unbekannter Eigenschaftsname ist kein Fehler.** RFC 5545 erlaubt
  `X-`-Eigenschaften und späteren IANA-Zuwachs. Das hat eine Folge, die beim
  Gegentest an einem echten RFC-Kalender aufgefallen ist: Verliert eine
  Fortsetzungszeile ihr führendes Leerzeichen, wird zum Beispiel aus
  `\r\n mailto:employee-A@example.com` die formal gültige Eigenschaft `mailto`
  — und keine der acht Prüfungen schlägt an. Der Fehler ist echt, das Werkzeug
  sieht ihn nicht. Wer ihn finden will, braucht eine Prüfung gegen die Liste
  der bekannten Eigenschaften; die ist nicht Teil dieser Mission.
...
beispiele/          Dreizehn Kalenderdateien, byte-genau, mit Absicht kaputt.
